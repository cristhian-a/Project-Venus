 ...
 * Provides a {@code BufferStrategy} implementation that uses multiple buffers to store data, and rotates them every on
 * every {@link #swap()} call, very similar to a ring buffer. Double or triple buffering are the most common strategy
 * usages for this class, but you can use any number of buffers you want (provided they are more than 1).
 * <p>
...
public final class MultiBufferStrategy<T extends Buffered> implements BufferStrategy<T> {
    ...
    /**
     * Creates a new {@code RingBuffer} with the given number of buffers. A {@link Supplier} is required to create each
    ...
    /**
     * Rotates the inner buffers. The back buffer becomes the surface one, the surface buffer is returned to the spare
     * pool, and the pool is rotated to provide the new back buffer. <br/>
    ...
    public void swap() {
        T newSurface  = back;
        T newBack = spare.poll();
        T newSpare = surface;

        if (newBack == null) throw new IllegalStateException("No spare buffers available.");

        back = newBack;
        spare.offer(newSpare);
        surface = newSurface;
    }
}
